...
public class PublicUtil {
    ...
    public static String addLineBreak(String content){
        if(TextUtils.isEmpty(content)){
            ...
            for(int i = 0; i < charArray.length; i++){
                sBuilder.append(charArray[i]).append("\n");
            }
    ...
    public static String getRandomHan() {
        return String.valueOf(han.charAt(new Random().nextInt(han.length())));
    }
    public static String getRandomHan(Random random) {
        return String.valueOf(han.charAt(random.nextInt(han.length())));
    }

    private static String cha = "abcdefghijklmnopqrstuvwxyz";
    public static String getRandomChar() {
        return String.valueOf(cha.charAt(new Random().nextInt(cha.length())));
    }
    public static String getRandomChar(Random random) {
        return String.valueOf(cha.charAt(random.nextInt(cha.length())));
    }

    private static String num = "0123456789";
    public static String getRandomNum() {
        return String.valueOf(num.charAt(new Random().nextInt(num.length())));
    }
    public static String getRandomNum(Random random) {
        return String.valueOf(num.charAt(random.nextInt(num.length())));
    }

    public static String getNickname(){
        Random radom = new Random();
        return "用户"+getRandomChar(radom)+getRandomChar(radom)+
                getRandomNum(radom)+getRandomNum(radom)+
                getRandomChar(radom)+getRandomChar(radom);
    }
}
